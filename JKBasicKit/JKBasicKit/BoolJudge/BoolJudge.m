...
+ (BOOL)isMobileNumber:(NSString *)string {
    ...
    if(![self isPureDigitalStr:string]) {
        return NO;
    }
    if(string.length != 11) {
        return NO;
...
+ (BOOL)isPhoneNumber:(NSString *)string {
    ...
    if(string.length != 11) {
        ...
        return [self isValidateByRegex:MOBILE Object:string];
    }
}
...
+ (BOOL)isIPAddress:(NSString *)string {
    ...
    BOOL rc = [self isValidateByRegex:regex Object:string];
...
+ (BOOL)authorityManagement:(PermissionType)mediaType superController:(UIViewController *)superController {
    ...
    if(mediaType == camera) {
        ...
        if(status == AVAuthorizationStatusRestricted || status == AVAuthorizationStatusDenied) {//判断操作是否被允许
            ...
            //提醒用户打开权限开关
            [self showAlert:@"相机" superController:superController];
            return NO;
        }
    }
        ...
        if (status == PHAuthorizationStatusRestricted || status == PHAuthorizationStatusDenied) {
            ...
            //提醒用户打开权限开关
            [self showAlert:@"相册" superController:superController];
            return NO;
        }
    }
        ...
        if (status == 0) {
            //提醒用户打开权限开关
            [self showAlert:@"麦克风" superController:superController];
        }
        return status;
    }
    else  {
        ...
        if([NetworkMonitoring sharedNetworkMonitoring].networkStatus == NetworkStatusNotReachable) {
            ...
            if (![CLLocationManager locationServicesEnabled] || type == kCLAuthorizationStatusDenied) {//判断定位操作是否被允许
                ...
                //提醒用户打开权限开关
                [self showAlert:@"定位" superController:superController];
...
+ (void)showAlert:(NSString *)mediaType superController:(UIViewController *)superController {
...
+ (void)showAlertStr:(NSString *)alertStr superController:(UIViewController *)superController {
